...
class GroupTokens(RecordProcessor):
    ...
    def group_tokens(self, material_mentions = None, property_mentions = None):
        ...
        material_names = []
        property_names = []
        if material_mentions is None:
            material_mentions = EntityList()
        if property_mentions is None:
        ...
        while i < span_length:
            ...
            if current_label != 'O' and i < span_length-1:
                ...
                if i < span_length-1:
                    next_label = self.spans[i+1].label
                i += 1
                ...
                # This will create a case in some cases where there is not a space as an artifice or tokenization as 1.80% will become 1.80 %
                joined_token = utils.token_post_processing(
                    ' '.join(cumulative_token))
                ...
                if current_label in self.material_entities and joined_token not in material_names:
                    # Condition handles cases where : part of IUPAC name
                    if '/' in joined_token or (':' in joined_token and not (len(joined_token) >= 3 and joined_token.index(':') != len(joined_token)-1 and joined_token[joined_token.index(':')+1] in string_numbers)):
                        token_list = []
                        token = ''
                        ...
                        for j, token in enumerate(token_list):
                            if token in [':', '/']:
                                grouped_spans.append(token_label(
                                    token, 'O', token_start+j, token_start+j))
                            else:
                                ...
                                for mat_mention in material_mentions.entity_list:
                                    if mat_mention.entity_name == token:
                                        ...
                                        grouped_spans.append(token_label(
                                            token, mat_label, token_start+j, token_start+j))
                                        break
                                else:
                                    material_names.append(token)
                                    material_mentions.entity_list.append(MaterialMention(
                                        entity_name=token, material_class=mat_label, coreferents=[token]))
                                    grouped_spans.append(token_label(
                                        token, current_label, token_start+j, token_start+j))
                        offset += len(token_list)-1

                    else:
                        # A material token cannon be a single bracket so this must happen when there is a tokenization issue
                        if joined_token in [')', '}']:
                            ...
                            material_mentions.entity_list.append(MaterialMention(
                                entity_name=joined_token, material_class=current_label, coreferents=[joined_token]))
                            grouped_spans.append(token_label(
                                joined_token, current_label, token_start, token_end))

                elif current_label == 'PROP_NAME' and joined_token not in property_names:
                    property_names.append(joined_token)
                    # Initialize one material record from base class
                    property_mentions.entity_list.append(PropertyMention(
                        entity_name=joined_token, coreferents=[joined_token]))
                    grouped_spans.append(token_label(
                        joined_token, current_label, token_start, token_end))
                # Assumption, there is never a good reason for a token to end in a dot, if this is an abbreviation, by convention we split the dot to a different token
                elif joined_token[-1] == '.':
                    # The dot is being separated out into a different token
                    grouped_spans.append(token_label(
                        joined_token[:-1], current_label, token_start, token_end))
                    # Implicitly assumes length of string greater than 1
                    grouped_spans.append(token_label(
                        joined_token[-1], 'O', token_end+1, token_end+1))
                    offset += 1
                else:
                    grouped_spans.append(token_label(
                        joined_token, current_label, token_start, token_end))
                ...
                grouped_spans.append(token_label(
                    self.spans[i].text, self.spans[i].label, token_start, token_start))
                i += 1
