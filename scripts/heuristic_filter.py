...
# generation_constant = 30 # Assume 30 tokens are generated on average per prompt
...
def add_to_filtered_paragrahs():
	print()
	
def heuristic_filter(property:str, publisher_directory:str):
	...
	for doi in poly_dois:
		...
		log.info(f"Processing {doi}")

		# processed_dois +=1
		filtration_dict['total_dois'] +=1

		paragraphs = PaperTexts().get_all(db, {'doi': doi})
		log.note(f'Number of paragraphs found: {len(paragraphs)}')
		...
		for para in paragraphs:
			...
			if found:
				...
				log.note(f"{para.id} did not pass the heuristic filter")
				# log.trace(para.text)
		
		if relevant_doi_paras>0:
			filtration_dict[f"{mode}_documents"] +=1
			log.warn(f'{doi} is relevant for {mode}.')
...
def log_run_info(publisher_directory):
    """
        Log run information for reference purposes.
        Returns a log Timer.
    """
    t1 = log.note("Heuristic Filter Run: {}", publisher_directory)
...
if __name__ == '__main__':
	...
	t1 = log_run_info(publisher_directory)
	
	heuristic_filter(property= property, publisher_directory= publisher_directory)
